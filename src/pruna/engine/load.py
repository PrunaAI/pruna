    ...
    **kwargs : Any
    ...
    try:  # Try to use pipeline for HF specific HQQ quantization
        model = HQQModelForCausalLM.from_quantized(
            model_path, device=smash_config.device, **filter_load_kwargs(HQQModelForCausalLM.from_quantized, kwargs)
        )
        ...
        model = AutoHQQHFModel.from_quantized(
            model_path, device=smash_config.device, **filter_load_kwargs(AutoHQQHFModel.from_quantized, kwargs)
        )

    return model


def load_quantized(model_path: str, smash_config: SmashConfig, **kwargs) -> Any:
    """
    ...
    **kwargs : Any
    ...
    if any(param.dtype.is_complex for param in model.parameters()):
        ...
        model = AutoAWQForCausalLM.from_quantized(
            model_path,
            device=smash_config.device_map if smash_config.device == "accelerate" else smash_config.device,
            **filter_load_kwargs(AutoAWQForCausalLM.from_quantized, kwargs),
        )
