...
def run_benchmark_vlm_batch_with_pred(
    ...
    *,
    ...
    if pred.shape[0] != len(prompts):
        raise ValueError(
            f"pred batch size {pred.shape[0]} does not match prompt batch size {len(prompts)}"
        )
    metric = build_vlm_benchmark_metric(
        metric_name,
        benchmark_key,
    ...
    *,
    ...
    for i, (prompts, auxiliaries) in enumerate(dm.test_dataloader()):
        ...
        if pred_i.shape[0] != len(prompts):
            raise ValueError(
                f"preds[{i}] batch size {pred_i.shape[0]} does not match prompt batch size {len(prompts)}"
            )
