...
class VieScoreMetric(StatefulMetric):
    ...
    **kwargs : Any
    ...
    Notes
    -----
    For GEditBench (arXiv:2504.17761), the reference pipeline uses a 2-criterion SC scoring
    (execution_success × over_editing_penalty) conditioned on both source and edited images.
    This implementation accesses only the edited image and instruction (single-criterion SC).
    Full GEditBench parity requires adding source-image support to the metric and dataset loader.
    """
    ...
    def update(self, x: List[Any] | torch.Tensor, gt: torch.Tensor, outputs: torch.Tensor) -> None:
        ...
        for i, image in enumerate(images):
            ...
            sem_score = get_score_from_response(sem_resp) * 10.0
            ...
            qual_score = get_score_from_response(qual_resp) * 10.0
