...
def is_file_exists(path: str | Path, filename: str) -> bool:
    """Return True if the file with the given filename exists under the provided path."""
    folder = Path(path)
...
def _normalize_prompts(
    prompts: str | List[str] | PrunaDataModule, split: str = "test", batch_size: int = 1, num_samples: int | None = None,
    fraction: float = 1.0, data_partition_strategy: str = "indexed", partition_index: int = 0, seed: int = 42
) -> Iterable[str]:
    """
    ...
    if isinstance(prompts, str):
        ...
        setattr(prompts, f"{split}_dataset", stratify_dataset(target_dataset, sample_size, seed, data_partition_strategy,
        partition_index))
...
def generate_videos(
    ...
    **model_kwargs : Any
    ...
    prompt_iterable = _normalize_prompts(
    prompts,
    split,
    batch_size=1,
    num_samples=num_samples,
    fraction=samples_fraction,
    data_partition_strategy=data_partition_strategy,
    ...
    def _seed_rng(x: int) -> torch.Generator:
        """Create a CPU torch.Generator seeded with the given integer."""
    ...
    for batch in prompt_iterable:
        ...
        for idx in range(unique_sample_per_video_count):
            ...
            if is_file_exists(save_dir, file_name):
                ...
                vid = sampler(prompt=prompt, seeder=_seed_rng(seed), **model_kwargs)
