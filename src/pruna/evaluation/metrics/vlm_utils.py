...
def yes_no_first_token_id_groups(tokenizer: Any) -> tuple[list[int], list[int]]:
    """Collect first subword token ids that can start a yes/no answer (next-token softmax mass).
    ...
    Args:
        tokenizer: Hugging Face ``PreTrainedTokenizer`` (or compatible ``encode``).

    Returns
    -------
        Two lists of distinct token ids for yes- and no-leaning first tokens, with overlap
        removed so each id is counted at most once (yes takes precedence on overlap).
    """
    yes_prefixes = (
        "Yes",
        " Yes",
