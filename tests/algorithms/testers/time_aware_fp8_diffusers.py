...
@pytest.mark.slow
class TestTimeAwareFp8Diffusers(AlgorithmTesterBase):
    """Test time-aware FP8 quantization for diffusers."""

    models = ["flux_tiny_random"]
    reject_models = ["llama_3_tiny_random"]
    allow_pickle_files = False
    algorithm_class = TimeAwareFp8Diffusers
    metrics = ["ssim"]
    artifacts_filename = TIME_AWARE_FP8_DIFFUSERS_ARTIFACTS_FILENAME
    artifacts_function_name = TIME_AWARE_FP8_DIFFUSERS_ARTIFACTS_FUNCTION_NAME
    hyperparameters = {
        "time_aware_fp8_diffusers_calibration_batches": 1,
        "time_aware_fp8_diffusers_calibration_num_inference_steps": 2,
    }
    ...
    def _assert_calibrated_linears(self, model: PrunaModel) -> None:
        """Assert calibrated TimeAwareFp8Linear layers share an initialized scale helper."""
        layers = self._fp8_linear_layers(model, TimeAwareFp8Linear)
        assert layers, "Expected TimeAwareFp8Linear layers"
        assert all(layer.input_initialized for layer in layers)
        assert layers[0].scale_helper.scales_initialized

    def post_smash_hook(self, model: PrunaModel) -> None:
        """Assert calibrated TimeAwareFp8Linear layers after smash."""
