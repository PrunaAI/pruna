...
def test_category_filter(dataset_name: str, category: str) -> None:
    ...
    assert all(
        aux.get("Category") == category or aux.get("Challenge") == category or aux.get("category") == category or aux.get("tag") == category
        for aux in auxiliaries
    )


@pytest.mark.slow
def test_geneval_with_category_filter() -> None:
