...
@pytest.mark.slow
def test_hps_with_category_filter():
    """Test HPS loading with category filter."""
    dm = PrunaDataModule.from_string(
        "HPS", category="anime", dataloader_args={"batch_size": 4}
    )
    dm.limit_datasets(10)
    batch = next(iter(dm.test_dataloader()))
    prompts, auxiliaries = batch
    ...
    assert len(prompts) == 4
    assert all(isinstance(p, str) for p in prompts)
    assert all(aux["category"] == "anime" for aux in auxiliaries)
