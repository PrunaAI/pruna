...
def test_metric_registration(metric_cls, metric_name):
...
@pytest.mark.parametrize(
    "metric_cls",
    [ImageRewardMetric, HPSMetric, HPSv2Metric],
)
def test_extract_prompts(metric_cls):
    """Test prompt extraction from different input types."""
    metric = metric_cls(device="cpu")
...
def test_score_image(metric_cls):
...
def test_update_and_compute(metric_cls):
...
def test_error_handling(metric_cls):
